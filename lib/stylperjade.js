...
function process(cssFiles, jadeFiles, options, cb) {
      ...
      async.parallelLimit(
        { css: function (cb) {
          async.eachLimit(files.css, os.cpus().length, processCssFile, function (err) {
            ...
            return cb(null,  _.union(cssClasses, cssIds))
          })
        }
          , jade: function (cb) {
          async.eachLimit(files.jade, os.cpus().length, processJadeFile, function (err) {
            ...
            return cb(null, _.union(jadeClasses, jadeIds))
          })
        }
        }
        , os.cpus().length
  ...
  function generateReport(type, count, cssClasses, jadeClasses, cssIds, report) {
    ...
    if (!options.allowIds) {
      for (var i = 0; i < cssClasses.length; i++) {
        if (cssClasses[i].name.indexOf('id') > -1) {
          ids.push(cssClasses[i])
        }
      }
      cssClasses = _.reject(cssClasses, function(el) { return el.name.indexOf('id') > -1; });
    }


    if (count > 0) {
      if (options.allowIds) {
        report.push(chalk.red('✘ ' + count + ' ' + type + ' classes/ids found'))
      } else {
        report.push(chalk.red('✘ ' + String(count - ids.length) + ' ' + type + ' classes found'))

      }

      if (ids.length > 0 && !options.allowIds) {
        report.push(chalk.red('✘ ' + ids.length + ' ids found in CSS files'))
      }
      ...
      if (options.allowIds) {
        report.push(chalk.green('✔ No ' + type + ' classes/ids found'))
      } else {
    ...
    function generateFileReport(type, fileType, collection) {
      ...
      if (collection.length > 0) {
        report.push('')
        if (options.allowIds) {
          report.push(chalk.cyan(collection.length + ' ' + type + ' classes/ids found in ' + fileType + ' files:'))
        } else {
  ...
  function processCssFile(file, cb) {
    ...
    function pushCssIds(idNames, position) {
      ...
      cssIds.push({ 'name': idNames, 'locations': [ location ] })
        ...
        jadeWalk(jadeParser(jadeLexer(data, file), file), function after(node) {
          ...
          if (node.type === 'Comment') {
            ...
            node.attrs.forEach(function (attr) {
              ...
              } else if (attr.name === 'id' && options.allowIds) {
                pushIds(jadeIds, attr.val, sourceFile, node.line)
              }
            })
          }
        })
  ...
  function pushIds(collection, value, file, line, column) {

    var idName = value.replace(/\'/g, '')
      , location = { 'file': file, 'line': line, 'column': column > 1 ? column : null }

    collection.push({ 'name': idName, 'locations': [ location ] })
